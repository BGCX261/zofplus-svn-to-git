	...
	.ui-autocomplete-input { margin: 0; padding: 0.48em 0 0.47em 0.45em; }
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                oTable =  $('#grillatrabajador').dataTable( {
                    "bJQueryUI": true,
                ...
                $("#grillatrabajador tbody").click(function(event) {
                ...
                $('#boton_agregar').click( function() {
                    $("#agregar_trabajador").validate();
                } );
                $('#boton_modificar').click( function() {
                    $("#modificar_trabajador").validate();
                    var actualizar = fnGetModificar( oTable );
                } );

                $('#boton_eliminar').click( function() {
                    $("#eliminar_trabajador").validate();
...
<script type="text/javascript" charset="utf-8">
    ...
    <body>
        ...
        <div id=titulo style="margin:2% 2%;font:80px calibri;color:#e4e4e4;position:absolute;">Trabajador</div>
        <div id=superior style="margin:10% 38%;font:17px calibri;color:wh <div id=titulo style="margin:2% 2%;font:80px calibri;color:#e4e4e4;position:absolute;">Trabajador</div>ite;position:absolute;">
            <ul>
...
<!--                <a href="#eliminar" name="modal" id="boton_eliminar">Eliminar</a><img src='img/eliminar.png'/> -->
        ...
        <div id="boxes" style="">
            ...
            <div id="agregar" class="window" style="position:absolute;background:url(img/hoja_papel.png);">
                <%@ include file='agregar_trabajador.jsp' %>
            </div><!-- fin formulario -->
        ...
        <div id="contenedor">
            ...
            <table class="display" id="grillatrabajador" style="width:100%;">
                <thead>
                    <tr>
                        <th>Rut</th>
                        <th>Nombre</th>
                ...
                <tbody>
                    <%
                                ...
                                RN_Persona trabajador= new RN_Persona();
                                List li = trabajador.ListarTrabajador();
                                Iterator it = li.iterator();
                                while (it.hasNext()) {
                                    Trabajador tra = (Trabajador) it.next();

                    %>
                    <tr>
                        <td><%=tra.getPersona().getPerRut() %></td>
                        <td><%=tra.getPersona().getPerNombre() %></td>
                        <td><%=tra.getPersona().getPerApellido() %></td>
                        <td><%=tra.getPersona().getPerDireccion() %></td>
                        <td><%=tra.getPersona().getPerFono() %></td>
                        <td><%=tra.getPersona().getPerEmail() %></td>
                        <td><%=tra.getPersona().getPerFnacimiento() %></td>
                    </tr>
                    <%}
                 ...
                 <div style="width:50%;font:16px calibri;color:#fff;margin-left:1%;"><fieldset><p>- <b>Modulo Familia:</b> su funcionalidad es poder registrar  las familias de los productos a ingresar o ya existentes en caso que no esten asociados.<br/>- Se pueden realizar operaciones como agregar, eliminar y modificar.<br/>- Para Modificaciones y Eliminaciones seleccione el galpon en la Grilla de Trabajo y luego presione la operacion que desee realizar.</p></fieldset></div>
